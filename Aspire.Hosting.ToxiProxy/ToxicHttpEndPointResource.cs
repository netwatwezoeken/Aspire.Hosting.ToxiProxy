    ...
    : Resource, IResourceWithParent<ToxiProxyResource>, IResourceWithEndpoints, IResourceWithWaitSupport
{
    ...
    public ToxicHttpEndPointResource(string name, ToxiProxyResource parent, int port, int? targetPort, IResourceBuilder<IResourceWithEndpoints> targetResource) : base(name)
    {
    ...
    private readonly List<ToxiResource> _toxicResources = [];
    
    internal void AddToxic(ToxiResource toxi)
    {
        _toxicResources.Add(toxi);
    }
    
    internal IReadOnlyList<ToxiResource> ToxicResources => _toxicResources;
}