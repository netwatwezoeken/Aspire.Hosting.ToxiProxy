...
Procedure for `plan-mode` step 3: decide where each slice's Gherkin will
...
after approval (step 6 of `plan-mode`).
...
2. **Detect the target project's BDD convention**

   Detect the target project's BDD convention — from the repo root, run:
```bash
python3 .opencode/scripts/detect_bdd_convention.py
```
